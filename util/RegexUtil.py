...
class RegexUtil(metaclass=Singleton):
    ...
    def create_regex_tipo_anotacao_jucesp(self):
        ...
        self.regex_tipo_anotacoes.append((re.compile("DESENQUADRA.{0,50}?\s*(EMPRESA\s*DE\s*PEQUENO\s*PORTE|E\.?P\.?P\.?)"),"Desenquadramento_EPP"))
        ...
        self.regex_tipo_anotacoes.append((re.compile("REVOG(ACAO|OU|AD[OA]).*?FALENCIA|FOI\s*REVOGAD[OA]\s*[OA]\s*(FALENCIA|O\s*DECRETO\s*(FALIMEN|DE\s*FALEN))"), "Revogacao_falencia"))
        self.regex_tipo_anotacoes.append((re.compile("(REVOG|ANUL)((O|(AR|ACAO)|(A(D[OA])?\-SE)))\s*D?[EOA]?\s*(DECRET(O|ACAO))?\s*D?[EOA]?\s*(FALENCIA|FALIMENTAR)|HOMOLOGO\s*O\s*ACORDO\s*(CELEBRADO)?\,?\s*REVOGANDO\s*O\s*DECRETO\s*(D[EA])?\s*(QUEBRA|FALENCIA|FALIMENTAR)|LEVANTO\s*A\s*FALENCIA\s*OUTRORA\s*DECRETADA"), "Revogacao_falencia"))
        ...
        self.regex_tipo_anotacoes.append((re.compile("ANOTACAO.*PROC\.?(ESSO)?|ANOTACAO.*JUIZ|OFICIO.*JUIZ|PENDENCIA\s*JUDICIAL"), "Anotacao_processo"))
        ...
        self.regex_tipo_anotacoes.append((re.compile("NOMEAD[OA]\s*SINDIC[OA]"),"Nomeada_sindica"))
    ...
    def create_regex_tipo_anotacoes_residual(self):
        ...
        self.regex_tipo_anotacoes_residual.append((re.compile("^RETIRA.*SOCIO"), "retirase_socio"))
    ...
    def create_regex_nome_abreviado(self):
        ...
        self.nome_abreviado_regex.append(
            (re.compile('\\bEDI(CAO|COES|FICA(CAO|COES)|FICIOS?|TORA?(CAO|IA(L|IS))?)\\b'), 'ED'))
    ...
    def limpa_texto_para_movimento_e_diario_para_quadro_credor(texto):
    ...
    def limpa_texto_diario_para_quadro_credor(texto_diario):
        texto = re.sub(
            'PUBLICACAO\s*OFICIAL\s*DO\s*TRIBUNAL\s*DE\s*JUSTICA.{0,50}LEI\s*FEDERAL[\s\,]*(Nº.{0,30})?ART\.\s*[0-9]+.?',
            '', texto_diario)
        texto = re.sub(
